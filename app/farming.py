...
def get_inventory_data (row):
    ...
    return {"id": row["id"],
            ...
            "inventory_char": find_item_in_inventory(f'SHARD_{gear_data["data"].get("characterId")}'.upper()),
...
def get_farming_table_html_rs():
    ...
    farming.sort_values(["rs", "red_delta"], ascending=[False, False] , inplace=True)
