...
def get_gold_from_api():
    ...
    events = r.json()
    current_time = time()
    current_gold_milestone = [event for event in events["data"] if "Gold" in event["name"] and event["startTime"] < current_time and event["endTime"] > current_time][0]["milestone"]
